...
export function setPluginVariableForMarkdownPreviewProcessor(
	snwPlugin: SNWPlugin,
) {
	plugin = snwPlugin;
}
...
export default function markdownPreviewProcessor(
	...
	if (
		...
		ctx.el.parentElement?.classList.contains(
	...
	try {
		ctx.addChild(
			new snwChildComponent(el, ctx.getSectionInfo(el), currentFile),
		);
	} catch (error) {
...
class snwChildComponent extends MarkdownRenderChild {
	...
	processMarkdown(): void {
		...
		if (
			...
			if (
				plugin.settings.enableRenderingBlockIdInMarkdown &&
				transformedCache?.blocks
			) {
				for (const value of transformedCache.blocks) {
					if (
						...
						const referenceElement = htmlDecorationForReferencesElement(
							...
							value.key,
							value.references[0]?.resolvedFile?.path.replace(
			...
			if (
				plugin.settings.enableRenderingEmbedsInMarkdown &&
				transformedCache?.embeds
			) {
				...
				this.containerEl
					...
					.forEach((element) => {
						let embedKey = parseLinkTextToFullPath(
							`${element.getAttribute("src")}`,
						);
						embedKey =
							embedKey === "" ? `${element.getAttribute("src")}` : embedKey;
						for (const value of transformedCache.embeds ?? []) {
							if (
								...
								const referenceElement = htmlDecorationForReferencesElement(
									...
									value.key,
									(value.references[0]?.resolvedFile?.path ?? "").replace(
										`.${value.references[0]?.resolvedFile?.extension ?? ""}`,
										"",
									),
									"snw-liveupdate",
									value.pos.start.line,
				...
				if (transformedCache?.headings && headerKey) {
					...
					for (const value of transformedCache.headings) {
						if (
							...
							const headerElement =
								this.containerEl.querySelector("h1,h2,h3,h4,h5,h6");
							if (headerElement) {
			...
			if (
				plugin.settings.enableRenderingLinksInMarkdown &&
				transformedCache?.links
			) {
				// biome-ignore lint/complexity/noForEach: <explanation>
				this.containerEl
					...
					.forEach((element) => {
						let link = parseLinkTextToFullPath(
						...
						for (const value of transformedCache.links ?? []) {
							if (
								value.references.length >= minRefCountThreshold &&
								(value.key === link || value?.original?.contains(link))
							) {
								const referenceElement = htmlDecorationForReferencesElement(
									value.references.length,
									"link",
